...
To begin, I will load the necessary packages and instacart data and set theme options.
...
Initial analysis shows `produce` to be the most frequently ordered from department, as visualized below: 
...
In 2010, `CA`, `CO`, `FL`, `MD`, `MA`, `NE`, `NJ`, `NY`, `NC`, `OH`, `PA`, `SC`, `TX`, and `WA` were observed at 7 or more locations.  
...
The state with the lowest average data value over the years, across locations, is `WY`.
...
I will aggregate across minutes to create a total activity variable for each day, and create a table showing these totals. 
